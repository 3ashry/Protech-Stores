...
// Kept module-scoped so the Apply button doesn't need to carry ~30KB of
// JSON in an inline onclick attribute (that was fragile and could silently
// break on any special character in a product name).
let _bulkPending = [];

// Local formatters/escapers so this file doesn't depend on app.js helpers
// having loaded (script tag order safety).
function _bulkEsc(s){
...
function _openBuyPricePreview() {
  try {
    if (typeof cache === 'undefined' || !Array.isArray(cache.products)) {
      alert('Products are still loading. Give it a second and try again.');
      return;
    }
    ...
    const unchanged = [];       // matched, no change needed
    ...
    // Stash for the Apply button (no huge inline JSON attribute).
    ...
    const chgRows = changes.length
      ? changes.slice(0, 500).map(c => `
          <tr>
            <td><span class="badge b-orange">${_bulkEsc(c.product.code)}</span></td>
            <td style="font-size:12px">${_bulkEsc(c.product.name || '')}</td>
            <td style="text-align:end">${_bulkFmt(c.oldBuy)}</td>
            <td style="text-align:end;color:#16a34a;font-weight:700">${_bulkFmt(c.newBuy)}</td>
            <td style="text-align:end;font-size:11px;color:${c.newBuy>c.oldBuy?'#dc2626':'#16a34a'}">${(c.newBuy - c.oldBuy).toFixed(2)}</td>
          </tr>`).join('')
        + (changes.length > 500 ? `<tr><td colspan="5" style="text-align:center;color:#666">…and ${changes.length - 500} more (all will be updated)</td></tr>` : '')
      : `<tr><td colspan="5" style="text-align:center;color:#666">Nothing to change</td></tr>`;
    ...
    const unmatchedBlock = unmatched.length
      ? `<div style="margin-top:12px;background:#fef3c7;border:1px solid #fbbf24;border-radius:8px;padding:10px">
           ...
           <div style="font-family:monospace;font-size:12px;max-height:140px;overflow:auto;white-space:pre-wrap;user-select:all">${_bulkEsc(unmatched.map(u => u.code).join('\n'))}</div>
         </div>` : '';

    const orphanBlock = dbNotInPdf.length
      ? `<div style="margin-top:8px;background:#f3f4f6;border:1px solid #d1d5db;border-radius:8px;padding:10px">
           ...
           <div style="font-family:monospace;font-size:11px;max-height:100px;overflow:auto;white-space:pre-wrap;user-select:all">${_bulkEsc(dbNotInPdf.slice(0,600).join('\n'))}${dbNotInPdf.length>600?'\n…':''}</div>
         </div>` : '';
      ...
      <div id="buy-price-mount" style="position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:99999;display:flex;align-items:center;justify-content:center;padding:16px">
        <div style="background:#fff;color:#111;max-width:900px;width:100%;max-height:92vh;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,.35);display:flex;flex-direction:column;overflow:hidden">
          <div style="display:flex;justify-content:space-between;align-items:center;padding:14px 18px;border-bottom:1px solid #e5e7eb">
            <strong style="font-size:16px">Bulk update buy prices — El Ashry 07/15</strong>
          ...
          <div style="padding:14px 18px;overflow:auto;flex:1">
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px">
              ...
              <div style="background:#f3f4f6;border-radius:8px;padding:10px;text-align:center"><div style="font-size:20px;font-weight:800;color:#374151">${dbNotInPdf.length}</div><div style="font-size:11px;color:#374151">Not in PDF</div></div>
            </div>
            <div style="max-height:50vh;overflow:auto;border:1px solid #e5e7eb;border-radius:8px">
            ...
            <div style="margin-top:14px;font-size:12px;color:#666">
              Formula: <code>buy_price = v21 × 0.95</code>, rounded to 2 decimals. Matching is exact code (case-insensitive, trimmed).
            </div>
          </div>
          <div style="display:flex;justify-content:flex-end;gap:8px;padding:12px 18px;border-top:1px solid #e5e7eb;background:#fafafa">
            ...
            <button type="button" id="buy-price-apply-btn" ${changes.length ? '' : 'disabled'}
                    onclick="_applyBuyPriceUpdates()"
                    style="padding:8px 14px;border:0;background:${changes.length ? '#f97316' : '#d1d5db'};color:#fff;border-radius:8px;cursor:${changes.length ? 'pointer' : 'not-allowed'};font-weight:700">
    ...
    console.error('Buy-price preview failed:', e);
